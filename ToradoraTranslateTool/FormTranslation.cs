...
{
    string currentFile;
    readonly string mainFilePath = Path.Combine(Application.StartupPath, "Data", "Translation.json");
    ...
    public FormTranslation()
    {
        ...
        typeof(DataGridView).InvokeMember("DoubleBuffered",
            ...
            null, dataGridViewStrings, [true]);

        typeof(DataGridView).InvokeMember("DoubleBuffered",
            BindingFlags.SetProperty | BindingFlags.Instance | BindingFlags.NonPublic,
            null, dataGridViewFiles, [true]);
        ...
        try
        {
            ...
            foreach (string path in Directory.GetDirectories(Path.Combine(Application.StartupPath, "Data", "Txt"))) directories.Add(Path.GetFileName(path)); // Get all directories with .obj and .txt files
            foreach (var path in Directory.GetDirectories(Path.Combine(Application.StartupPath, "Data", "Obj"))) directories.Add(Path.GetFileName(path));
    ...
    private void SaveProgress()
    {
        ...
        string translationPercent = Math.Round((translatedCount * 100.0) / dataGridViewStrings.Rows.Count, 1).ToString();
